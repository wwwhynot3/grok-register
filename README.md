...
| 按钮 | 对应命令 | 功能 |
|---|---|---|
| 📊 状态总览 | `/status` | 完整状态总览(全部板块) |
| 🌊 池水位 | `/pool` | 池水位 vs 阈值 + 判定 |
| 🔄 刷新 | `/refresh` | 凭据 fresh/过期/失败/永久失效 |
| 📝 注册 | `/register` | 注册历史与成功率 |
| 🎨 铸造 | `/mint` | 铸造数量/新增/独立用户 |
| ⚡ API 调用 | `/api` | API 成功率 + 状态码 + 模型分布 |
| 💰 余额 | `/balance` | LuckMail/YesCaptcha 余额 |
| 🖥 节点 | `/nodes` | 出口节点健康 + 出口 IP |
| ⚙️ 服务 | `/services` | systemd 单元状态 |
| 🔁 重授权 | `/reauth` | 待重授权数 + 守护状态 |
| 🔔 告警 | `/alerts` | 告警通道配置 |
