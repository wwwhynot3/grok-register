...
from config import BROWSER_UA as user_agent
_proxy = os.getenv("GROK_PROXY") or "http://127.0.0.1:7897"
# TLS 指纹: 默认最新 Chrome。旧版本(如 chrome120/124/131)已被 CF 拦截(2026-08-14 实测 403)。
# 指纹会随时间过时, 可随时用 GROK_IMPERSONATE 覆盖。
_impersonate = os.getenv("GROK_IMPERSONATE", "chrome")
...
def register_single_thread(email_provider: str = "gptmail"):
    ...
    while not stop_event.is_set():
        try:
            with requests.Session(impersonate=_impersonate, proxies=PROXIES) as session:
                # 预热连接
                    ...
                    if res.status_code == 200:
                        ...
                        if sso:
                            with file_lock:
                                ...
                                with open(os.path.join(KEYS_DIR, "grok.txt"), "a") as f: f.write(sso + "\n")
                                with open(os.path.join(KEYS_DIR, "accounts.txt"), "a") as f: f.write(f"{email}:{password}:{sso}\n")
...
def main():
    ...
    parser.add_argument("--email-provider", default=os.getenv("EMAIL_PROVIDER", "gptmail"), help="邮箱提供商：gptmail/luckmail/luckmail_order/mailtm/mailnest/gmail，可逗号分隔回退链（如 luckmail_order,gmail）")
    ...
    with requests.Session(impersonate=_impersonate, proxies=PROXIES) as s:
        try:
